...
| Dropped span context propagation | ⚠️ | `Decision.DROP` currently returns `INVALID_SPAN`, so trace context is not preserved |
...
| Identical instrument aggregation | ⚠️ | Separate `create_*()` calls with the same identity produce separate `Metric` objects |
...
| Requirement | Status | Notes |
|-------------|--------|-------|
| `emit(log_record)` | ✅ | Accepts OTel `LogRecord`-compatible records and keyword fields |
| Stdlib logging bridge | ✅ | `OtelBridgeLoggingHandler` converts Python `logging.LogRecord` values |
...
| `service.instance.id` | ❌ | Not generated by default |
...
|--------|-------------|-------|
| **Traces** | ~85% | Core functionality works; missing AddLink after creation and non-recording sampled-out spans |
| **Metrics** | ~85% | All instruments implemented; missing ExponentialHistogram and identical instrument aggregation |
...
- Sampled-out spans should preserve trace context by returning a `NonRecordingSpan` instead of `INVALID_SPAN`.
- Identical metric instruments should be registered once and aggregate into one exported `Metric`.
- `service.instance.id` is not generated by default, and forked-process uniqueness still needs validation.
- Exception recording is basic for spans and not yet exposed as structured `exception.*` log attributes.
